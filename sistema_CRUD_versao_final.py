...
while x == True:
    ...
    if escolha == 3:
        ...
        if produto_alterado in dicionario[loja]:
            escolha_2 = int(input("1 - alterar o estoque\n2 - alterar o preço\nFaça sua escolha:"))
            if escolha_2 == 1:
                nova_quantidade = int(input("Quantidade:"))
                dicionario[loja][produto_alterado]['quantidade'] += nova_quantidade
                print("novo estoque de {0}: {1}\n".format(produto_alterado,dicionario[loja][produto_alterado]["quantidade"]))
                
            if escolha_2 == 2:
                novo_preco = float(input("novo preço:"))
                while novo_preco < 0:
                    print("O novo preço não pode ser negativo")
                    novo_preco = int(input("Por favor digitar novamente o novo preço:"))
            dicionario[loja][produto_alterado]['preco'] = novo_preco
            print("novo preco de {0}: {1}\n".format(produto_alterado,dicionario[loja][produto_alterado]['preco']))
        else:
            print("Elemento não encontrado")
    if escolha == 4:
        ...
        if escolha_3 == 2:
            ...
            for key in dicionario[loja]:
                if dicionario[loja][key]['quantidade'] < 0:
                    ...
                    print('Não ha estoque negativos')
